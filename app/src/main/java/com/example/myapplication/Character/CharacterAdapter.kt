...
class CharacterAdapter(private val Characterlist: ArrayList<CharacterDataClass>) :
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: CharacterDataClass, position: Int) {
            ...
            itemView.setOnClickListener {
                val context = it.context as CharacterActivity
                val intent = android.content.Intent(itemView.context, AddCharacterActivity::class.java).apply {
                    putExtra("edit_mode", true)
                    putExtra("name", item.name)
                    putExtra("position", position)
                }
                context.startActivityForResult(intent, CharacterActivity.REQUEST_CODE_CHAR_EDIT)
            }
        }
    }
