...
class SeoiActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 더미 데이터 띄우기
        val adapter = SeoiAdapter(Characterlist)
        ...
        btnAddPlus.setOnClickListener {
            // 캐릭터 추가 액티비티로 이동
            val intent = android.content.Intent(this, AddSeoiActivity::class.java)
            ...
            if(requestCode == REQUEST_CODE_CHAR_ADD) {
                ...
                val position = data?.getIntExtra("position", -1)
                    if(position != null && position != -1 && name != null) {
                        ...
                        character.name = name
                        Characterlist[position] = character
                }
            }
