package com.example.myapplication.Seoi
...
class SeoiAdapter(private val Characterlist: ArrayList<CharacterDataClass>) :
    RecyclerView.Adapter<SeoiAdapter.ViewHolder>() {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: CharacterDataClass, position: Int) {
            ...
            itemView.setOnClickListener {
                val context = it.context as SeoiActivity
                val intent = android.content.Intent(itemView.context, AddSeoiActivity::class.java).apply {
                    putExtra("edit_mode", true)
                    putExtra("name", item.name)
                    putExtra("position", position)
                }
                context.startActivityForResult(intent, SeoiActivity.REQUEST_CODE_CHAR_EDIT)
            }
        }
    }
